...
> **Project tokens take precedence.** The design system in `style.css` defines the authoritative palette (`--accent: #6e40c9`, `--sidebar-bg: #0d1117`, etc.). These general guidelines apply to new styles and custom additions — never override existing tokens to comply with them.
...
- Purple or magenta (exception: project accent `--accent` and sidebar `--sidebar-bg` are part of the design system — do not change them)
